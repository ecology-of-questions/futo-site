/**
 * ocrTesseractLocal.ts(旧 ocr.ts)
 * ------------------------------------------------------------
 * 【2026-09-21、Decision Log 0198: 標準経路から外した】実写真での比較
 * 検証(Decision Log 0196・0197)の結果、Google Cloud Visionの方が
 * 実用的だったため、Fieldnoteの通常のOCR実行経路は`ocr.ts`
 * (Google Cloud Vision版)に切り替えた。このファイルはブラウザ内
 * Tesseract.js実装を、実機での動作確認が済むまで戻せる形でそのまま
 * 保持するためのもの——どこからもimportされていない(比較ツール
 * `scripts/ocr-compare/`は別途Node上で直接tesseract.jsを使っており、
 * このファイルには依存しない)。`public/vendor/tesseract/`の自己ホスト
 * 済みアセットも、このファイルのために未削除のまま残している。
 * 実機確認後、別PRでこのファイル・vendorアセット・tesseract.js依存の
 * 要否を再検討する。
 *
 * 以下は元のドキュメントコメント(このファイルが標準経路だった当時のもの)。
 * ------------------------------------------------------------
 * ocr.ts
 * ------------------------------------------------------------
