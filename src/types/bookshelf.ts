 ...
 *
 * 【「本棚に本を持ち寄る」機能の追加に伴い`contributed`を新設
 * (2026-09-11、Decision Log 0137)】`/bookshelf`に、訪問者が本を
 * 提案できるフォームを追加した。フォームからの送信は自動掲載せず、
...
export interface BookEntry {
  ...
  relatedUrl?: string;
  /**
   * 訪問者が「本棚に本を持ち寄る」フォームから提案し、「ふ、と」が
   * 確認・承認したうえで掲載した本であることを示すフラグ。trueの本は
   * BookshelfFullList.astroで「持ち寄られた本」という控えめなラベルを
   * 表示する。承認は運営がこの配列に手動で追加する形で行うため、
