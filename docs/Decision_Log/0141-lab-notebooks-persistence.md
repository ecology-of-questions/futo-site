...
- 日付: 2026-09-13
- 状態: 採用(一部 → Decision Log 0142・0143で更新); UI部分は
  superseded(→ 0178。2026-09-18、`/participate/[slug]`から
  「書かれたこと」一覧・書き込みフォームを撤去。API・D1・データ構造は
  このDecision Logの記載のまま存続)

**2026-09-13追記:** 本番設定前のセキュリティレビューにより、
以下2点はDecision Log 0142で更新された(この節は削除せず残す)。
1. `ip_hash`の生成方法(鍵無しSHA-256 → secretを使うHMAC-SHA256)。
   下記「スパム対策」節の記述は更新前の内容。
