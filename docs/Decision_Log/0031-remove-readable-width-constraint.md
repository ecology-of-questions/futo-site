...
- 日付: 2026-08-15
- 状態: 一部上書き済み(→ 0089)。「文字が多くて読む気をなくす」との
  フィードバックを受け、2026-09-04にResearch Statement・About・
  研究断面01の記事本文にだけ、読みやすい行幅を再導入した。Hero・
  Research一覧等の短文・カード類は、このDecision Logの「画面
  いっぱいに使う」方針のまま変更していない。詳細はDecision Log
  0089参照。(Decision Log 0008・0012・0026・0029が積み重ねてきた
  「読みやすい行幅」という制約を撤回した、という本エントリの
  経緯自体はそのまま有効)
