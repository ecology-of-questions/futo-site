...
**ステータス: superseded (→ 0188)。** プロジェクトオーナーから方針修正の指示を受け、「ページ自体を非公開にする」のではなく「操作可能なデモだけを外し、紹介ページ自体は公開を維持する」方針に変更した。この方針転換の理由・実装内容はDecision Log 0188に記録する。本エントリは差し替えず、判断の記録としてそのまま残す。
