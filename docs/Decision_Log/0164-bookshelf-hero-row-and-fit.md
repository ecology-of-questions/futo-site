...
- 日付: 2026-09-17
- 状態: superseded (→0166)。この直後のDecision Log 0165(1冊ずつ
  表示化)を経て、プロジェクトオーナーから「見にくくなった」との
  指摘を受け、Decision Log 0166で本エントリの変更(タイトル行の
  横並び化・余白圧縮・著者名のinline統合・font-size調整)を含め、
  Decision Log 0154時点の表示に差し戻した。この判断・実装の記録
  として本エントリは削除せず残す。
- 関連: Decision Log 0161(/participateの1画面圧縮)・0163(/participate
  のHero行横並び化)。同じ考え方を`/bookshelf`に適用した新規PR。
  Decision Log 0165(この直後に適用した1冊ずつ表示化)・0166
  (この変更の差し戻し)。
